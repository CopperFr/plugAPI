...
class PlugAPI extends EventEmitter3 {
     ...
     *
     * @example
     * bot.getUpdatedUser(uid, (err, data) => console.log(err, data));
     */
    getUpdatedUser(uid, callback) {
        if (!Object.is(typeof callback, 'function')) {
            this.logger.error('PlugAPI', 'Missing callback for getUpdatedUser');
            return;
        }
        store(this)._queueREST('GET', 'users/' + uid, undefined, callback, true);
    }
