    ...
    addSongToPlaylist(pid, sdata, append, callback) {
        ...
        this.getPlaylist(pid, (playlist) => {
            ...
            const callbackWrapper = (err, data) => {
                if (!err) {
                    console.log(data);
                    //playlist = store(this)._editPlaylistLength(playlist, data.data[0].count);
                }
                if (Object.is(typeof callback, 'function')) {
                    return callback(err, data);
                }
    ...
    removeSongFromPlaylist(pid, mid, callback) {
        ...
        this.getPlaylist(pid, (playlist) => {
            ...
            const callbackWrapper = (err, data) => {
                if (!err) {
                    console.log(data);
                    //playlist = store(this)._editPlaylistLength(playlist, data.data.length);
                }
                if (Object.is(typeof callback, 'function')) {
                    return callback(err, data);
                }
