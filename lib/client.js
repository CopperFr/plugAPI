...
class PlugAPI extends EventEmitter3 {
    constructor(authenticationData, callback) {
         ...
         * @param {Boolean|Number} incrementOrCount Whether to increment, decrement or new playlist count.
         * @returns {Array} The playlist is returned with the modified count
         * @private
         */
        store(this)._editPlaylistLength = (playlist, incrementOrCount) => {
            if (Object.is(typeof incrementOrCount, 'undefined') || Object.is(incrementOrCount, true)) {
                playlist.count++;
            } else if (Object.is(incrementOrCount, false)) {
                playlist.count--;
            } else if (Object.is(typeof incrementOrCount, 'number')) {
                playlist.count = incrementOrCount;
            }
            return playlist;
        };
    ...
    /**
     * Update a user missing properties.
     * @param {RESTCallback} callback Callback function
     *
     * @example
     * bot.updateUser(uid, (err, data) => console.log(err, data));
     */
    updateUser(uid, callback) {
        const callbackWrapper = (requestError, data) => {
            if (!requestError) {
                data[0].i = data[0].id;
                store(this)._room.updateUser(data[0]);
            }
            if (Object.is(typeof callback, 'function')) {
                return callback(requestError, data[0]);
            }
        };

        store(this)._queueREST('GET', 'users/' + uid, undefined, callbackWrapper, true);
    }
     ...
     * bot.getTimeElapsed();
     */
    getTimeElapsed(clamp) {
        if (!store(this)._room.getRoomMeta().slug || store(this)._room.getMedia() == null) {
            return -1;
        }
        const elapsed = DateUtilities.getSecondsElapsed(store(this)._room.getStartTime());
        if (clamp == false) {
            return elapsed;
        } else {
            return Math.min(store(this)._room.getMedia().duration, elapsed);
        }
    }
     ...
     * bot.addSongToPlaylist(123456, [{cid: '789abc', format: 1, image: 'https://i.ytimg.com/blah', duration: 300, title: 'My song', author: 'My song author'}])
       bot.addSongToPlaylist(123456, [36546471])
     */
    addSongToPlaylist(pid, sdata, append, callback) {
        console.log(`addSongToPlaylist ${pid}`);
        ...
        if (!sdata.every(value => Object.is(typeof value, 'number'))) {
        ...
        this.getPlaylist(pid, (playlist) => {
            ...
            const callbackWrapper = (err, data) => {
                if (!err) {
                    playlist = store(this)._editPlaylistLength(playlist, data[0].count);
                }
                if (Object.is(typeof callback, 'function')) {
                    return callback(err, data);
                }
            };
            console.log("_queueREST");
    ...
    removeSongFromPlaylist(pid, mid, callback) {
        ...
        this.getPlaylist(pid, (playlist) => {
            ...
            const callbackWrapper = (err, data) => {
                if (!err) {
                    playlist = store(this)._editPlaylistLength(playlist, data.length);
                }
                if (Object.is(typeof callback, 'function')) {
                    return callback(err, data);
                }
